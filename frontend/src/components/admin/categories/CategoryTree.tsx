...
const CategoryItem: React.FC<{ cat: Category; onEdit: (c: Category) => void; onDelete: (id: string) => void }> = ({ cat, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden">
      <div className="p-5 relative">
        {cat.isFeatured && (
          <span className="absolute top-3 left-48 bg-gradient-to-r from-yellow-400 to-amber-500 text-white text-xs font-semibold px-3 py-1 rounded-full shadow-sm">
            ⭐ Featured
          </span>
        )}
        ...
        <div className="flex items-start justify-between gap-4">
          <div onClick={() => onEdit(cat)} className="flex-1 cursor-pointer group">
            <div className="flex items-center gap-2 mb-2">
              <h2 className="font-semibold text-lg text-gray-800 group-hover:text-blue-600 transition-colors">{cat.name}</h2>
              {cat.children.length > 0 && (
                <span className="bg-blue-100 text-blue-700 text-xs font-medium px-2 py-0.5 rounded">{cat.children.length} sub</span>
              )}
            </div>
