...
const CardProduct: React.FC<CardProductProps> = ({ product }) => {
  const discountedPrice = product.sale && product.sale > 0 ? product.price - (product.price * product.sale) / 100 : null;

  return (
    <Link to={`/products/${product.slug}`}>
      <div className="relative max-w-sm rounded-lg overflow-hidden transform transition duration-300 hover:scale-105">
        <img className="w-full max-h-80" src={product.defaultVariantId?.images?.[0] || product.image} alt={product.name} />
        ...
        <div className="p-4">
          ...
          {product.is_new && <span className="absolute top-4 left-4 bg-blue-500 text-white text-xs font-bold px-2 py-1 rounded-br-lg">MỚI</span>}
