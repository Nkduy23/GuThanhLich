...
const LoginForm: React.FC<LoginFormProps> = ({ onSuccess }) => {
  ...
  const { error, loading, handleSubmit } = useAuthForm<LoginResponse>("http://localhost:3000/api/auth/login", (data) => onSuccess(data.token, ""));
  ...
  return (
    <>
      ...
      <form onSubmit={onSubmit}>
        ...
        <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white py-2 rounded-md hover:bg-blue-700 transition duration-300 disabled:opacity-50">
